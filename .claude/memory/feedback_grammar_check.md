...
- Beyond grammar fixes, also offer a **better/more natural rewrite** even when the sentence is already grammatically correct (confirmed 2026-05-08). User is actively trying to improve fluency, not just avoid errors — so suggest 1–3 native-sounding alternatives with a one-line reason each. Skip only if the original is genuinely already idiomatic. Applies both to inline corrections done by main Claude AND to delegated `english-grammar-checker` runs (the subagent's prompt was updated the same day to enforce this).
